 ...
 *
 * Unrecognised *server* messages are deliberately not shown — see the matching note in
 * stores/layouts.ts. Errors we threw ourselves still pass through; `instanceof Error` is
 * what tells them apart, because supabase-js returns a plain object.
 */
function describeError(error: unknown, fallback: string): string {
  ...
  if (message) {
    console.error('Unrecognised short link error:', message)
  }
  return fallback
}
